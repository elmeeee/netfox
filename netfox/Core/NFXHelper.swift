...
extension HTTPModelShortType {

    public init(contentType: String) {
        if NSPredicate(format: "SELF MATCHES %@", "^application/(vnd\\.(.*)\\+)?json$").evaluate(
            with: contentType)
        {
            self = .JSON
...
extension NFXColor {
    ...
    // MARK: - Primary Accent Colors (Modern Vibrant Palette)

    class func NFXOrangeColor() -> NFXColor {
        // Modern vibrant purple-pink gradient color
        return NFXColor.init(netHex: 0x8B5CF6)  // Vibrant Purple
    }

    class func NFXAccentColor() -> NFXColor {
        // Secondary accent - Electric Blue
        return NFXColor.init(netHex: 0x3B82F6)
    }

    class func NFXPinkAccentColor() -> NFXColor {
        // Tertiary accent - Hot Pink
        return NFXColor.init(netHex: 0xEC4899)
    }

    // MARK: - Status Colors (Success/Error with modern palette)

    class func NFXGreenColor() -> NFXColor {
        // Modern success green - Emerald
        return NFXColor.init(netHex: 0x10B981)
    }

    class func NFXDarkGreenColor() -> NFXColor {
        // Darker emerald for text
        return NFXColor.init(netHex: 0x059669)
    }

    class func NFXRedColor() -> NFXColor {
        // Modern error red - Rose
        return NFXColor.init(netHex: 0xEF4444)
    }

    class func NFXDarkRedColor() -> NFXColor {
        // Darker rose for text
        return NFXColor.init(netHex: 0xDC2626)
    }

    class func NFXYellowColor() -> NFXColor {
        // Warning/Pending - Amber
        return NFXColor.init(netHex: 0xF59E0B)
    }

    // MARK: - Neutral Colors (Modern Gray Scale)

    class func NFXStarkWhiteColor() -> NFXColor {
        // Soft white for backgrounds
        return NFXColor.init(netHex: 0xF9FAFB)
    }

    class func NFXDarkStarkWhiteColor() -> NFXColor {
        // Light gray for secondary backgrounds
        return NFXColor.init(netHex: 0xE5E7EB)
    }

    class func NFXLightGrayColor() -> NFXColor {
        // Medium gray for borders
        return NFXColor.init(netHex: 0xD1D5DB)
    }

    class func NFXGray44Color() -> NFXColor {
        // Dark gray for text
        return NFXColor.init(netHex: 0x6B7280)
    }

    class func NFXGray95Color() -> NFXColor {
        // Very light gray for subtle backgrounds
        return NFXColor.init(netHex: 0xF3F4F6)
    }

    class func NFXBlackColor() -> NFXColor {
        // Rich black for primary text
        return NFXColor.init(netHex: 0x111827)
    }

    class func NFXDarkModeBackgroundColor() -> NFXColor {
        // Dark mode background
        return NFXColor.init(netHex: 0x1F2937)
    }

    class func NFXCardBackgroundColor() -> NFXColor {
        // Card background with slight tint
        return NFXColor.init(netHex: 0xFFFFFF)
    }

    // MARK: - Gradient Helper

    class func NFXGradientStartColor() -> NFXColor {
        // Gradient start - Purple
        return NFXColor.init(netHex: 0x8B5CF6)
    }

    class func NFXGradientEndColor() -> NFXColor {
        // Gradient end - Pink
        return NFXColor.init(netHex: 0xEC4899)
    }
}
...
extension URLRequest {
    ...
    func getNFXBody() -> Data {
        return httpBodyStream?.readfully() ?? URLProtocol.property(forKey: "NFXBodyData", in: self)
            as? Data ?? Data()
    }
...
struct NFXPath {
    ...
    static func createNFXDirIfNotExist() {
        do {
            try FileManager.default.createDirectory(
                at: nfxDirURL, withIntermediateDirectories: true, attributes: nil)
        } catch let error {
    ...
    static func deleteNFXDir() {
        guard FileManager.default.fileExists(atPath: nfxDirURL.path, isDirectory: nil) else {
            return
        }
    ...
    static func deleteOldNFXLogs() {
        ...
        guard
            ...
            let fileEnumarator = fileManager.enumerator(
        ...
        for case let fileURL as URL in fileEnumarator {
            if fileURL.lastPathComponent == oldSessionLogName
                || fileURL.lastPathComponent.hasPrefix(oldRequestPrefixName)
            {
...
extension String {
    ...
    func appendToFileURL(_ fileURL: URL) {
        ...
        if #available(iOS 13.4, macOS 10.15.4, *) {
            do {
                ...
                print(
                    "[NFX]: Failed to append [\(self.prefix(128))] to \(fileURL), trying to create new file - \(error.localizedDescription)"
                )
                write(to: fileURL)
            }
        } else {
    ...
    private func write(to fileURL: URL) {
        do {
            ...
            print(
                "[NFX]: Failed to save [\(self.prefix(128))] to \(fileURL) - \(error.localizedDescription)"
            )
        }
    }
