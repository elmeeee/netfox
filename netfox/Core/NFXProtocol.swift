...
open class NFXProtocol: URLProtocol {
    ...
    private class func canServeRequest(_ request: URLRequest) -> Bool {
        ...
        guard
            !NFX.sharedInstance().getIgnoredURLs().contains(where: { absoluteString.hasPrefix($0) })
        else {
            return false
        }
    ...
    override open func startLoading() {
        ...
        if let urlString = request.url?.absoluteString {
            print("[netfox API Log] Full URL: \(urlString)")
        }
...
extension NFXProtocol: URLSessionDataDelegate {
    ...
    public func urlSession(
        ...
        client?.urlProtocol(
            ...
            cacheStoragePolicy: NFX.swiftSharedInstance.cacheStoragePolicy)
